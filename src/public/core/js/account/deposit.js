...
class BankTransferManual {
    ...
    `;
        ...
        let isSubmitting = false; // Biến trạng thái

        // Sự kiện submit khi nhấn vào nút XÁC NHẬN
        $('#modal-container').on('click', '.btn-submit-deposit', function () {
            const button = $(this);

            // Kiểm tra xem đã gọi submit chưa
            if (isSubmitting) {
                return; // Nếu đã gọi, không làm gì thêm
            }

            // Đặt trạng thái đang submit
            isSubmitting = true;

            // Vô hiệu hóa nút để ngăn không cho nhấn lại
            button.prop('disabled', true);

            // Thực hiện phương thức submitDeposit
            initMethodDeposit.submitDeposit(button);
        });
    ...
    handleDepositModalButtonClick() {
        ...
        $('.toto-top-up__open-deposit-modal-button').on('click', function () {
            ...
            const selectedIdBank = $('.toto-top-up__payment-method-item--selected img').attr('alt');
            ...
            if (!selectedIdBank) {
                toastr.info('Vui lòng chọn ngân hàng')
                return;
            }

            if (!amount) {
                toastr.info('Vui lòng nhập số tiền')
                return;
            }

            // Kiểm tra số tiền tối thiểu
            if (amount < 50000) {
                toastr.info('Số tiền tối thiểu là 50.000 VND.')
                return;
            }
    ...
    submitDeposit(element) {
        ...
        if (!bank || !nameDeposit || !numberDeposit || !bankDeposit) {
            toastr.infor('Vui lòng nhập đầy đủ thông tin trước khi nhấn xác nhận nha!')
            return;
        }

        if (amountDeposit <= 0) {
            toastr.warning('Số tiền nạp không hợp lệ!')
            return;
        }
...
class BankTransferAuto {
    ...
    submitDeposit(element) {
        ...
        if (!bank) {
            toastr.error('Vui lòng nhập đầy đủ thông tin trước khi nhấn xác nhận hen!')
            return;
        }

        if (amountDeposit <= 0) {
            toastr.error('Số tiền nạp không hợp lệ!')
            return;
        }
        ...
        $.ajax({
            ...
            if (response.status) {
                toastr.success("Vui lòng thanh toán trong cửa sổ hiện lên!")
                element.hide();
